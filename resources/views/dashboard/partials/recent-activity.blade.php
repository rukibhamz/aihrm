<div class="card p-6 h-full flex flex-col">
    ...
    <div class="space-y-4">
        ...
        @forelse($recentActivities as $activity)
            <div class="flex items-start gap-3 pb-4 border-b border-zinc-100 dark:border-zinc-800 last:border-0 last:pb-0">
                <div class="w-8 h-8 bg-{{ $activity['color'] }}-100 dark:bg-{{ $activity['color'] }}-950/50 rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1">{{ $activity['time']->diffForHumans() }}</p>
                </div>
                @if(isset($activity['status']))
                    <span class="px-2 py-1 text-xs rounded
                        {{ $activity['status'] === 'pending' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-950 dark:text-yellow-300' : '' }}
                        {{ $activity['status'] === 'approved' ? 'bg-green-100 text-green-800 dark:bg-green-950 dark:text-green-300' : '' }}
                        {{ $activity['status'] === 'rejected' ? 'bg-red-100 text-red-800 dark:bg-red-950 dark:text-red-300' : '' }}
                        {{ $activity['status'] === 'completed' ? 'bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-300' : '' }}
                    ">
